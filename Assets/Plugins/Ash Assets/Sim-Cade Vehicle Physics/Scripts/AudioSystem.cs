...
namespace Ashsvp
{
    public class AudioSystem : MonoBehaviour
    {
        ...
        {
            var speed = gearSystem.carSpeed;

            var angularSpeed = GetComponent<Rigidbody>().angularVelocity.magnitude;

            //engineSound.pitch = Mathf.Lerp(minPitch, maxPitch, Mathf.Abs(speed + (verticalInput + angularSpeed/10) * 20) / maxspeed);
            var enginePitch = Mathf.Lerp(minPitch, maxPitch,
                Mathf.Abs(speed) / gearSystem.gearSpeeds[Mathf.Clamp(gearSystem.currentGear, 0, 4)]);
