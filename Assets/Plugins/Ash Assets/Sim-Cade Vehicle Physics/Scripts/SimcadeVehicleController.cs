...
namespace Ashsvp
{
    public class SimcadeVehicleController : MonoBehaviour
    {
        ...
        [HideInInspector]
        public float steerInput, accelerationInput;

        [HideInInspector]
        public bool brakeInput;

        [HideInInspector]
        public float rearTrack, wheelBase, ackermennLeftAngle, ackermennRightAngle;
        ...
        public void Init(IVehicleController controller)
        {
        ...
        void AddAcceleration(float accelerationInput)
        {
            ...
            if (!brakeInput && localVehicleVelocity.z < MaxSpeed)
            {
        ...
        void brakeLogic()
        {
            ...
            float deltaSpeed = brakeAcceleration * (brakeInput ? 1 : 0) * Time.fixedDeltaTime * Mathf.Clamp01(Mathf.Abs(localSpeed));
            deltaSpeed = Mathf.Clamp(deltaSpeed, -MaxSpeed, MaxSpeed);
            if (localSpeed > 0)
            {
                rb.linearVelocity -= transform.forward * deltaSpeed;
